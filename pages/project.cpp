...
route("/api/media/add", media_add) {
  ...
  for (const auto &p : parts) {
    if (p.name == "folder") {
      ...
      filename = trim(std::string(p.data.begin(), p.data.end()));
    } else if (p.name == "file") {
      file_data = p.data;
      if (!p.filename.empty()) {
        filename = p.filename; // use original filename if available
      }
    }
  }
...
route("/project/template", main_template) {
  Server.SSR("public/views/template.htpp", connection);
  return OK(connection);
}
